...
why the gopls application depends on the cmp package:
	...
	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps uw/pkg/x/tools/gopls |
		digraph somepath uw/pkg/x/tools/gopls github.com/google/go-cmp/cmp

Show which packages in x/tools depend, perhaps indirectly, on the callgraph package:

	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps uw/pkg/x/tools/... |
		digraph reverse uw/pkg/x/tools/go/callgraph
