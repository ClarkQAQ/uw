...
executable lives at uw/pkg/x/tools/cmd/godoc.
...
```
godoc -templates=$GOPATH/src/uw/pkg/x/tools/godoc/static -http=:6060
```
...
2) Run `go generate uw/pkg/x/tools/godoc/static` so `static/static.go` picks
up the change.

3) Run `go install uw/pkg/x/tools/cmd/godoc` so the compiled `godoc` binary
picks up the change.
