...
// To run:
//
// $ cd $GOPATH/src/uw/pkg/x/tools/gopls
...
func validateHardcodedVersion(version string) error {
	const debugPkg = "uw/pkg/x/tools/gopls/internal/lsp/debug"
...
func validateGoModFile(goplsDir string) error {
	...
	hash := string(stdout)
	// Find the uw/pkg/x/tools require line and compare the versions.
	var version string
	for _, req := range gomod.Require {
		if req.Mod.Path == "uw/pkg/x/tools" {
			version = req.Mod.Version
			break
		}
	}
	if version == "" {
		return fmt.Errorf("no require for uw/pkg/x/tools")
	}
	...
	if !strings.HasPrefix(hash, last) {
		return fmt.Errorf("uw/pkg/x/tools pseudoversion should be at commit %s, instead got %s", hash, last)
	}
	return nil
}
