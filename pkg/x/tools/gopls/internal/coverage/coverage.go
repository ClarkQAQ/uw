...
func main() {
	...
	parg := "uw/pkg/x/tools/gopls/internal/lsp/..."
...
func checkCwd() {
	...
	// we expect to be at the root of uw/pkg/x/tools
	cmd := exec.Command("go", "list", "-m", "-f", "{{.Dir}}", "uw/pkg/x/tools")
	buf, err := cmd.Output()
	buf = bytes.Trim(buf, "\n \t") // remove \n at end
	if err != nil {
		log.Fatal(err)
