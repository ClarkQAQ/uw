...
import (
	...
	"uw/pkg/x/tools/internal/goroot"
...
// NeedsLocalXTools skips t if the uw/pkg/x/tools module is replaced and
...
func NeedsLocalXTools(t testing.TB) {
	...
	cmd := Command(t, "go", "list", "-f", "{{with .Replace}}{{.Dir}}{{end}}", "-m", "uw/pkg/x/tools")
	out, err := cmd.Output()
	if err != nil {
	...
	if want := "uw/pkg/x/tools"; modulePath != want {
